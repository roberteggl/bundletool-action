...
function pickJarAsset(assets) {
    const preferred = assets.find((asset) => /^bundletool-all-.*\.jar$/i.test(asset.name));
    if (preferred) {
        return preferred;
    }
    ...
    throw new DownloadError('No .jar asset found on the google/bundletool release.');
}
async function fetchJson(url) {
    ...
    try {
        ...
        throw new DownloadError(`Failed to reach GitHub Releases API (${url}): ${error instanceof Error ? error.message : String(error)}`);
    }
    if (response.status === 404) {
        throw new DownloadError(`Bundletool release not found: ${url}`);
    }
    if (!response.ok) {
        throw new DownloadError(`Unexpected GitHub API response from ${url}: ${response.status} ${response.statusText}`);
    }
 ...
 */
export async function resolveRelease(source, logger) {
    if (source.url) {
        const version = source.version === 'latest' ? 'custom' : normalizeVersion(source.version);
